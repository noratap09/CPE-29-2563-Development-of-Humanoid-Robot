void walk(int num_walk,float walk_len,float walk_deg,float body_shift,float bia_body_shift,float bia_hit,int speed_per_f,String ck)
{
  if (num_walk > 0)
  {
    int n_sample = 20;
    ...
    float body_shift_L = body_shift+0.009;
    ...
    for (int i_walk = 1; i_walk <= num_walk; i_walk++)
    {
      ...
      for (int i = 1; i <= n_sample; i++)
      {
          IK_L( mapfloat(i, 1, n_sample, walk_len, 0.00) -x_bia , mapfloat(i, 1, n_sample, -body_shift_R, body_shift_L*0.9) - y_bia , h_com , 0, 0, 0, 0, 90  + constrain( mapfloat(i,1,n_sample,walk_deg,0.00) ,0.00,walk_deg) );
          IK_R( mapfloat(i, 1, n_sample, 0.00, -walk_len) -x_bia , mapfloat(i, 1, n_sample, -body_shift_R, body_shift_L*0.9) - y_bia , h_com , mapfloat(i, 1, n_sample,bia_body_shift_R*0.2, 0.00), 0, 0, 0, 120 -  constrain( mapfloat(i,1,n_sample,0.00,walk_deg) ,0.00,walk_deg) );
          delay(t_delay);
      }
      delay(t_delay_by_pos);
      ...
      for (int i = 1; i <= n_sample; i++)
      {
          ...
          IK_R( mapfloat(i, 1, n_sample, -walk_len, 0.00) -x_bia , mapfloat(i, 1, n_sample,body_shift_L*0.9,body_shift_L) - y_bia , (h_com - bezier(mapfloat(i, 1, n_sample, 0.00, 0.50))) , 0, 0, 0, 0, 120 - constrain( mapfloat(i,1,n_sample,walk_deg,0.00) ,0.00,walk_deg));
          delay(t_delay*0.5);
      }
    delay(t_delay_by_pos);
    ...
    for (int i = 1; i <= n_sample; i++)
    {
        IK_L( mapfloat(i, 1, n_sample, 0.00, -walk_len) -x_bia , mapfloat(i, 1, n_sample, body_shift_L, -body_shift_R*0.9) - y_bia , h_com , mapfloat(i, 1, n_sample, -bia_body_shift_L*0.2, 0.00), 0, 0, 0, 95 - constrain( mapfloat(i,1,n_sample,0.00,walk_deg) ,walk_deg,0.00));
        IK_R( mapfloat(i, 1, n_sample, walk_len, 0.00) -x_bia , mapfloat(i, 1, n_sample, body_shift_L, -body_shift_R*0.9) - y_bia , h_com , 0, 0, 0, 0, 120 + constrain( mapfloat(i,1,n_sample,walk_deg,0.00) ,walk_deg,0.00));
        delay(t_delay);
    }
    delay(t_delay_by_pos);
    ...
    for (int i = 1; i <= n_sample; i++)
    {
        ...
        IK_R( 0.00 -x_bia, mapfloat(i, 1, n_sample, -body_shift_R*0.9, -body_shift_R) - y_bia , h_com , mapfloat(i, 1, n_sample, 0.00, bia_body_shift_R), 0, 0, -bia_hit_R , 120);
        delay(t_delay*0.2); 
    }
    delay(t_delay_by_pos);
